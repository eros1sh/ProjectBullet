...
namespace ProjectBullet.Avalonia.ViewModels
{
    public class ConfigsViewModel : ViewModelBase
    {
        ...
        public async Task CreateAsync(ConfigForCreationDto dto)
        {
            ...
            allConfigs.Insert(0, newConfigVM);
            ApplyFilter();
        ...
        {
            allConfigs.ForEach(c => c.UpdateViewModel());
            base.UpdateViewModel();
        }

        public void CreateCollection()
        {
            allConfigs = configService.Configs.Select(c => new ConfigViewModel(c)).ToList();
            ApplyFilter();
        }
        ...
        private void ApplyFilter()
        {
            var filtered = allConfigs
                .Where(c => ConfigsFilter(c))
                .OrderByDescending(c => c.IsFavorite);
            ConfigsCollection = new ObservableCollection<ConfigViewModel>(filtered);
        }

        public bool ConfigsFilter(ConfigViewModel item) => item.Config
    ...
    public class ConfigViewModel : ViewModelBase
    {
        ...
        public Bitmap Icon => Images.Base64ToBitmap(Config.Metadata.Base64Image);
