...
namespace RuriLib.Functions.Http
{
    internal abstract class HttpRequestHandler
    {
        ...
        protected static void ApplyBrowserProfile(BotData data, Options.HttpRequestOptions options)
        {
            if (options.BrowserProfile == BrowserProfileName.None) return;

            var profile = BrowserProfileRepository.GetByName(options.BrowserProfile.ToString());
            if (profile == null) return;

            // Apply profile headers (custom headers take priority)
            foreach (var header in profile.Headers)
            {
                if (!options.CustomHeaders.ContainsKey(header.Key))
                {
                    options.CustomHeaders[header.Key] = header.Value;
                }
            }

            // Set User-Agent from profile if not already set by custom headers
            ...
            if (options.BrowserProfile == BrowserProfileName.Random && data.Providers.RandomUA != null)
            {
                try
                {
                    var ua = data.Providers.RandomUA.Generate();
                    options.CustomHeaders["User-Agent"] = ua;
                }
                catch { /* Fall back to profile UA */ }
            }
        }
    }
}
