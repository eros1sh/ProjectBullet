...
namespace ProjectBullet.Native.Updater.Helpers;
    ...
    public static async Task EnsureOb2NativeNotRunningAsync()
    {
        ...
        await AnsiConsole.Status()
            .StartAsync("[yellow]Checking if OpenBullet 2 is running...[/]", async ctx =>
            {
                ...
                while (Process.GetProcessesByName("ProjectBullet.Native").Length > 0 && timeout > TimeSpan.Zero)
                {
                ...
                if (Process.GetProcessesByName("ProjectBullet.Native").Length > 0)
                {
                    isRunning = true;
                }
            });
